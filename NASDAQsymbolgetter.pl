...
# It used a static symbols list but now AMEXsymbolgetter.pl will
...
use IO::Socket;
my @symbols;
my $output = "/home/mark/stockquotes/NASDAQstocksymbols.txt";

open OUTPUT, ">$output" or die "Cannot open filehandle OUTPUT ($output): $!\n";

my $server  = 'www.nasdaq.com';
my $port    = '80';
my $request = "/screening/companies-by-name.aspx?letter=0&exchange=nasdaq&render=download";
